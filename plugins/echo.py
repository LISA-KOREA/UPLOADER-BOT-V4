...
# Enhanced IMPERSONATION_LIST for better compatibility
IMPERSONATION_LIST = [
    "Chrome-120", 
    "Firefox-135", 
    "Edge-101", 
...
async def echo(bot, update):
    ...
    chk = await bot.send_message(
        ...
        text="🔍 Testing with different browser impersonations...",
        disable_web_page_preview=True,
        reply_to_message_id=update.id,
        parse_mode=enums.ParseMode.HTML
    )

    # Enhanced yt-dlp with IMPERSONATION_LIST functionality
    response_json = None
    
    # Try each impersonation target
    for target in IMPERSONATION_LIST:
        try:
            ydl_opts = {
                "outtmpl": "%(title)s.%(ext)s",
                "format": "best",
                "extractor_args": {"generic": [f"impersonate={target}"]},
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # Get video info to verify compatibility
                info = ydl.extract_info(url, download=False)
                # Store the response for later use
                response_json = info
                
            print(f"✅ Worked with impersonation: {target}")
            await chk.edit_text(f"✅ Successfully connected with {target} browser simulation")
            break  # stop once successful
            
        except Exception as e:
            print(f"⚠️ Failed with {target}: {e}")
            await chk.edit_text(f"⚠️ Failed with {target}, trying next browser...")
            continue

    # If no impersonation worked, try fallback command method
    if response_json is None:
        await chk.edit_text("🔄 Using fallback method...")
        ...
        if t_response:
            ...
            # fallback for nonnumeric port
            inline_keyboard = []
            inline_keyboard.append([
                InlineKeyboardButton(
                    "📁 ᴍᴇᴅɪᴀ",
                    callback_data="video=OFL=ENON".encode("UTF-8")
                )
            ])
